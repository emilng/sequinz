 ...
 * @function
 */
const name = (note) => get$3(note).name;
/**
 ...
 * simplify("B#4") // => "C5"
 */
const simplify$1 = nameBuilder(true);
/**
...
function get$4(name) {
    return typeof name === "string"
        ? index$2[name.toLowerCase()] || NoMode
        : name && name.name
            ? get$4(name.name)
            : NoMode;
}
const mode = deprecate("Mode.mode", "Mode.get", get$4);
/**
...
const entries = deprecate("Mode.mode", "Mode.all", all$2);
/**
...
function notes(modeName, tonic) {
    return get$4(modeName).intervals.map((ivl) => transpose(tonic, ivl));
}
...
function distance(destination, source) {
    const from = get$4(source);
    const to = get$4(destination);
...
export { index$1$1 as Mode, index$1 as Note, index$3 as Scale };
